    ...
    *,
    ...
    def make_legacy_visual_spec() -> Dict[str, Any]:
        ...
        return {
            ...
            "subtitle": f"{ticker} | Generated by AnalystIQ",
    ...
    def add_text(
        ...
        *,
        ...
        paragraph.font.name = font_heading if size >= 22 else font_body
    ...
    def add_header(slide, spec: Dict[str, Any], slide_no: int):
        ...
        headline_rule = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, margin, Inches(1.36), Inches(1.45), Inches(0.03))
        headline_rule.fill.solid()
        headline_rule.fill.fore_color.rgb = accent_blue
        headline_rule.line.fill.background()
        if spec.get("visual_emphasis"):
            add_text(
                slide,
                plain(spec.get("visual_emphasis"), 95),
                Inches(9.05),
                Inches(0.3),
                Inches(3.25),
                Inches(0.35),
    ...
    def add_metrics(slide, metrics: List[Dict[str, Any]], left, top, width, height):
        ...
        for idx, metric in enumerate(metrics[:count]):
            ...
            if metric.get("delta"):
                delta_text = str(metric.get("delta", ""))
                delta_color = danger_red if delta_text.strip().startswith("-") else success_green
                add_text(slide, delta_text, x + Inches(0.15), top + Inches(0.88), card_w - Inches(0.3), Inches(0.32), size=9, color=delta_color)
    ...
    def add_speaker_notes(slide, spec: Dict[str, Any]):
        notes_text = plain(spec.get("speaker_notes", ""), 650)
        if notes_text:
    ...
    def render_title_slide():
        ...
        add_text(
            slide,
            deck_spec.get("subtitle") or f"{ticker} | Generated by AnalystIQ",
            margin,
            Inches(3.0),
            Inches(7.8),
            Inches(0.45),
        ...
        add_card(slide, Inches(8.55), Inches(1.35), Inches(3.9), Inches(3.6))
        add_text(slide, "Investment Thesis", Inches(8.9), Inches(1.75), Inches(3.2), Inches(0.35), size=11, color=accent_blue, bold=True, all_caps=True)
        add_text(slide, plain(deck_spec.get("investment_thesis") or summary_text, 155), Inches(8.9), Inches(2.2), Inches(3.2), Inches(1.9), size=14, color=text_slate)
        add_text(slide, datetime.now().strftime("%Y-%m-%d"), margin, Inches(6.6), Inches(2.0), Inches(0.3), size=10, color=muted_gray)
        add_footer(slide, 1)

    def render_hero_slide(spec: Dict[str, Any], slide_no: int, chart_path: Optional[str]):
        slide = blank_slide()
        add_header(slide, spec, slide_no)
        add_card(slide, margin, Inches(1.45), Inches(12.1), Inches(4.95))
        add_text(
            slide,
            plain(spec.get("takeaway") or deck_spec.get("investment_thesis"), 150),
            Inches(0.9),
            Inches(1.95),
            Inches(7.2),
            Inches(1.2),
        ...
        add_bullets(slide, spec.get("bullets", []), Inches(0.9), Inches(3.4), Inches(6.9), Inches(2.8), size=14, max_items=3)
        add_card(slide, Inches(8.25), Inches(1.85), Inches(4.15), Inches(4.1))
        if chart_path:
            add_fit_picture(slide, chart_path, Inches(8.45), Inches(2.05), Inches(3.75), Inches(3.65))
        else:
            add_text(
                slide,
                plain(spec.get("visual_emphasis") or "Decision context for committee discussion", 90),
                Inches(8.55),
                Inches(3.25),
                Inches(3.55),
                Inches(1.0),
    ...
    def render_chart_focus_slide(spec: Dict[str, Any], slide_no: int, chart_path: Optional[str]):
        slide = blank_slide()
        add_header(slide, spec, slide_no)
        ...
        add_text(slide, "Takeaway", panel_left + Inches(0.28), chart_top + Inches(0.28), Inches(2.75), Inches(0.28), size=9, color=accent_blue, bold=True, all_caps=True)
        add_text(slide, plain(spec.get("takeaway", ""), 115), panel_left + Inches(0.28), chart_top + Inches(0.68), Inches(2.75), Inches(1.25), size=15, color=brand_navy, bold=True)
    ...
    def render_risk_matrix_slide(spec: Dict[str, Any], slide_no: int):
        slide = blank_slide()
        add_header(slide, spec, slide_no)
        add_card(slide, margin, Inches(1.45), Inches(12.1), Inches(4.95))
        add_text(
            slide,
            plain(spec.get("takeaway") or "Risk priority map for downside control.", 130),
            Inches(0.92),
            Inches(1.85),
            Inches(11.2),
            Inches(0.6),
        ...
        matrix_w = Inches(8.1)
        matrix_h = Inches(3.3)
        grid = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, matrix_left, matrix_top, matrix_w, matrix_h)
        grid.fill.solid()
        grid.fill.fore_color.rgb = card_color
        grid.line.color.rgb = line_color
        vline = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE,
            matrix_left + int(matrix_w / 2),
            matrix_top,
            Inches(0.01),
            matrix_h,
        )
        vline.fill.solid()
        vline.fill.fore_color.rgb = line_color
        vline.line.fill.background()
        hline = slide.shapes.add_shape(
            MSO_SHAPE.RECTANGLE,
            matrix_left,
            matrix_top + int(matrix_h / 2),
            matrix_w,
            Inches(0.01),
        )
        hline.fill.solid()
        hline.fill.fore_color.rgb = line_color
        hline.line.fill.background()
        add_text(slide, "HIGH IMPACT", Inches(9.85), Inches(2.55), Inches(2.4), Inches(0.24), size=8, color=muted_gray, bold=True)
        add_text(slide, "LOW IMPACT", Inches(9.85), Inches(5.4), Inches(2.4), Inches(0.24), size=8, color=muted_gray, bold=True)
        add_text(slide, "HIGH PROBABILITY", Inches(4.05), Inches(5.93), Inches(2.1), Inches(0.24), size=8, color=muted_gray, bold=True, align=PP_ALIGN.CENTER)
        add_text(slide, "LOW PROBABILITY", Inches(1.65), Inches(5.93), Inches(2.2), Inches(0.24), size=8, color=muted_gray, bold=True, align=PP_ALIGN.CENTER)
        risk_bullets = [plain(point, 75) for point in spec.get("bullets", []) if plain(point, 75)][:3]
        add_card(slide, Inches(9.75), Inches(2.95), Inches(2.65), Inches(2.75))
        add_bullets(slide, risk_bullets, Inches(9.9), Inches(3.12), Inches(2.35), Inches(2.3), size=11, max_items=3)
        add_speaker_notes(slide, spec)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for idx, spec in enumerate(slides[:9], start=2):
            if not isinstance(spec, dict):
                continue
            layout = spec.get("layout_type", "two_column")
            chart_enabled_layouts = {"hero", "thesis", "chart_focus", "metrics_dashboard", "two_column"}
            chart_path = resolve_chart_path(spec, chart_paths, used_chart_refs) if layout in chart_enabled_layouts else None
            if layout == "thesis":
                render_thesis_slide(spec, idx, chart_path)
            elif layout == "hero":
                render_hero_slide(spec, idx, chart_path)
            elif layout == "chart_focus":
                render_chart_focus_slide(spec, idx, chart_path)
            elif layout == "metrics_dashboard":
                render_metrics_slide(spec, idx, chart_path)
            elif layout == "risk_matrix":
                render_risk_matrix_slide(spec, idx)
